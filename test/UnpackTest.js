...
function buildFile(prefix, rowCount, colCount, lastNewline) {
  const f = [...Array(rowCount).keys()]
    .map(i => {
      return [...Array(colCount).keys()]
        .map(j => prefix + i.toString() + j.toString())
        .join("\t");
    })
    .join("\n");
...
function buildTest(tmpDir, fileCount, rowsPerFile, colCount, lastNewline, cb) {
  ...
  const toCreate = [...Array(fileCount).keys()].map(el => {
    return {
      i: el,
      n: path.join(tmpDir, `${buildFileName(el, fileCount)}.gz`)
    };
  });
  mkdirp(tmpDir, err => {
...
describe("Unpack", () => {
  ...
  describe("loadSchema", () => {
    let unpack = new Unpack(
      {},
      { saveLocation: "fake", unpackLocation: "fake2" },
      logger
    );
    it("should work to load a schema", done => {
      unpack.schemaLocation = path.join(
        __dirname,
        "./fixtures/mockSchema.json"
      );
  ...
  describe("addTitleAndUnzip", () => {
    const schema = {
      schema: {
        one: {
          tableName: "one",
          columns: [{ name: "id" }, { name: "type" }, { name: "name" }]
        },
        two: {
          tableName: "two",
          columns: [{ name: "letter" }, { name: "type" }]
        },
        three: {
          tableName: "three",
          columns: [{ name: "sound" }]
    ...
    it("should write out files to where we expect", done => {
      ...
      let unpack = new Unpack(
        { filter: ["one", "two"] },
        { saveLocation: mockDump, unpackLocation: d },
        logger
      );
      mkdirp(d, err => {
        assert.ifError(err);
  ...
  describe("run", () => {
    it("should ensure the unpack location is created", done => {
      ...
      let unpack = new Unpack(
        {},
        { saveLocation: "fake", unpackLocation: d },
        logger
      );
      unpack.loadSchema = function(cb) {
        cb(null, { mocked: true });
      };
      unpack.addTitleAndUnzip = function(s, sourceDir, outDir, cb) {
        cb();
      };
      unpack.run(err => {
        assert.ifError(err);
