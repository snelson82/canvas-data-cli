...
var mockApiObjects = module.exports = {
  ...
  buildDumpFile(opts) {
    ...
    const artifacts = tables.map((tableName) => mockApiObjects.buildDumpArtifact(opts.tableOpts[tableName] || {
      tableName
    }));
    return _.defaults(opts, {
      dumpId: '1234',
  ...
  buildDumpHistoryEntry(opts) {
    ...
    var files = _.fill(Array(numFiles), 0).map((v, index) => mockApiObjects.buildDumpHistoryFile({
      filename: `filename-${index}.tar.gz`
    }));
    return _.defaults(opts, {
      dumpId: '1234',
      sequence: 0,
  ...
  buildDumpHistoryFile(opts) {
    return _.defaults(opts, {
      url: 'http://myapi.com/table/filename-1.tar.gz',
      filename: 'filename-1.tar.gz'
    });
  }
