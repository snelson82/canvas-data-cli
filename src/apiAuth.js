...
var apiAuth = {
  ...
  signRequest: function(key, secret, requestOpts, opts) {
    ...
    var dateVal =
      requestOpts.headers.Date || opts.date || new Date().toUTCString();
    ...
    var reqOpts = {
      ...
      contentMD5:
        opts.contentMD5 ||
        (requestOpts.headers && requestOpts.headers["Content-MD5"]
          ? requestOpts.headers["Content-MD5"]
          : null)
    };
  ...
  determineContentType: function(requestOpts, opts) {
    ...
    if (requestOpts.json && typeof requestOpts.json === "object")
      return "application/json";
    if (requestOpts.body && typeof requestOpts.body === "string")
      return "text/plain";
    if (requestOpts.body && Buffer.isBuffer(requestOpts.body))
