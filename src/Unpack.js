...
class Unpack {
  constructor(opts, config, logger) {
    ...
    this.outputLocation = path.resolve(
      process.cwd(),
      config.unpackLocation || "./unpackedFiles"
    );
  ...
  addTitleAndUnzip(schema, sourceDir, outputDir, cb) {
    ...
    this.logger.debug(
      `will unpack ${toUnpack.map(p => p.tableName).join(",")}`
    );
    ...
    fs.readdir(inputDir, (err, files) => {
      ...
      const streamCreators = files.map(f => {
        ...
        return function() {
          ...
          return fs
            .createReadStream(path.join(inputDir, f))
            .pipe(gunzip)
            .pipe(split())
            .pipe(
              mapS((item, cb) => {
