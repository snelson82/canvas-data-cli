...
class Sync {
        ...
        async.mapLimit(
          ...
          (err, results) => {
            ...
            if (splitResults.erroredFiles.length) {
              this.logger.warn(
                `${splitResults.erroredFiles.length} files failed to download, please try running the sync again, if this error persists, open a ticket. No files will be cleaned up`
              );
              this.logger.warn(splitResults.erroredFiles);
              return cb(
                new Error(
                  "failed to download some files, try running sync again"
                )
              );
            }
  ...
  splitResults(results) {
    ...
    var newDownloaded = results
      .filter(res => res.didDownload)
      .map(res => res.filename);
    var cached = results
      .filter(res => !res.error && !res.didDownload)
      .map(res => res.filename);
    return {
      erroredFiles,
      newDownloaded,
  ...
  logResults(splitResults) {
    if (splitResults.cached.length === splitResults.results.length) {
      ...
      this.logger.info(
        `downloaded ${splitResults.newDownloaded.length} new files out of ${splitResults.results.length} total files`
      );
  ...
  getSync(cb) {
    ...
    this.api.getSync((err, toSync) => {
      ...
      if (err) return cb(err);
      if (toSync.incomplete)
        this.logger.warn(
          `Could not retrieve a full list of files! Some incremental data will be missing!`
        );
  ...
  downloadSchema(schemaVersion, cb) {
    mkdirp(this.saveLocation, err => {
      ...
      this.api.getSchemaVersion(schemaVersion, (err, schema) => {
        if (err) return cb(err);
        fs.writeFile(
          path.join(this.saveLocation, "schema.json"),
  ...
  processFile(fileInfo, cb) {
    ...
    this.fileExists(filename, (err, exists) => {
      ...
      this.logger.info(
        `already have ${fileInfo.filename}, no need to redownload`
      );
      return cb(null, {
        error: null,
  ...
  cleanupFiles(downloadedFiles, cb) {
    ...
    glob(
      ...
      (err, files) => {
        ...
        async.map(
          toRemove.map(name => path.join(this.saveLocation, name)),
          fs.unlink,
          cb
        );
      }
